...
func NewFrom[T any](sequence collections.Collection[T]) Queue[T] {
	return Queue[T]{sequence}
}

func New[T any]() Queue[T] {
	return Queue[T]{array.New[T]()}
}

func (queue Queue[T]) Empty() bool {
	return queue.sequence.Size() == 0
}

func (queue Queue[T]) Push(value T) {
	queue.sequence.Append(value)
}

func (queue Queue[T]) Peek() T {
	return queue.sequence.Get(0)
}
